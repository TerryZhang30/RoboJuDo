...
class HumanxLoopPolicy(Policy):
    ...
    def __init__(self, cfg_policy: HumanxLoopPolicyCfg, device):
        ...
        self.motion_fps: int = int(motion['fps'])
        self.motion_length_s = (motion['dof'].shape[0] - 1) / self.motion_fps

        # Ball / object trajectory from motion data
        self._obj_pos: np.ndarray | None = motion.get('obj_pos', None)
        self._obj_rot_quat: np.ndarray | None = motion.get('obj_rot_quat', None)
        self._ball_release_frame: int = int(motion.get('stand_frame', motion['dof'].shape[0]))
        if self._obj_pos is not None:
            logger.info(
                f"Loaded ball trajectory: {self._obj_pos.shape[0]} frames, "
    ...
    def get_observation(self, env_data, ctrl_data):
        ...
        # Provide ball target position to the pipeline / env
    ...
    def _get_ball_info(self) -> dict | None:
        """Ball is always released immediately into free dynamics.
        Initial position comes from BallCfg.init_pos (set in env config)
        or the hands-midpoint fallback."""
